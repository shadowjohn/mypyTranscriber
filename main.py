...
VERSION = 1.6
message = '''
mypyTranscriber CLI Version {VERSION}

Usage : 
  main.exe [input wav] [output txt] [wav lang] [output txt lang]
  main.exe [wavFile] [txtOutput]  
  main.exe [wavFile] [txtOutput] zh zh-TW
  main.exe [wavFile] [txtOutput] zh-HK zh-TW
  main.exe [wavFile] [txtOutput] zh zh-CN
  main.exe [wavFile] [txtOutput] en zh-TW
  main.exe [wavFile] [txtOutput] en zh-CN

'''
message = message.replace("{VERSION}",str(VERSION))
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
      ...
      default_lang = "zh-TW"
      default_output_lang = "zh-TW" 
      if len(sys.argv) == 4:
        default_lang = sys.argv[3]
      if len(sys.argv) == 5:
        default_lang = sys.argv[3]
        default_output_lang = sys.argv[4]      
      #print("Lang: %s\n" % (default_lang));  
      ...
      data = my.file_get_contents(outputFileSRT)
      data = data.replace(" ","\r\n")
      if my.in_array(my.strtoupper(default_output_lang),["ZH","ZH-CN","ZH_CN"])==True:
        data = trad2simple(data);
      else:
        data = simple2trad(data);          
      my.file_put_contents(outputFileSRT,my.s2b(data))
      print(data); 
    #sys.exit(main())
